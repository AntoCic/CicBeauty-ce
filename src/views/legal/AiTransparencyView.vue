...
useChangeHeader('Trasparenza AI', { name: 'home' })
...
<template>
  <div class="legal-page container-fluid pb-t overflow-auto h-100" :style="bgStyle">
    <HeaderApp title="Trasparenza AI" :to="{ name: 'home' }" />

    <article class="legal-card">
      <p class="legal-updated">Ultimo aggiornamento: {{ legalConfig.legalLastUpdated }}</p>

      <h2>Fornitore e modello</h2>
      <p>
        Le funzioni AI del sito usano {{ legalConfig.aiProvider }} con modello minimo {{ legalConfig.aiModel }} tramite
        API server-side.
      </p>

      <section class="legal-html" v-html="legalConfig.aiTransparencyBodyHtml" />

      <h2>Contatti</h2>
      <p>
        Per richieste su AI e protezione dati: {{ legalConfig.privacyEmail }}.
      </p>
