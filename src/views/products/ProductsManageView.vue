...
const filteredItems = computed(() => {
  ...
  const source = [...productStore.itemsActiveArray].sort((a, b) => a.title.localeCompare(b.title, 'it'))
...
function goToEdit(productId: string) {
  router.push({ name: 'ProductEditView', params: { id: productId } })
}
</script>
  ...
  <div class="container-fluid pb-t overflow-auto h-100" :style="bgStyle">
    ...
    <section class="compact-list">
      ...
      <div v-if="canManage" class="vstack gap-1">
        ...
        <p v-if="!groupedFilteredItems.length" class="text-muted small mb-0 mt-1">Nessun prodotto.</p>
      </div>
    </section>
  </div>
</template>
