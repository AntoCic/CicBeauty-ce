...
<template>
  <div class="container-fluid pb-t overflow-auto h-100" :style="bgStyle">
    ...
    <Form :validation-schema="filterSchema" :initial-values="{ search: '' }" v-slot="{ values }" class="mb-3">
      <Field name="search" class="form-control" placeholder="Cerca trattamenti..." />
      ...
      <div class="vstack gap-2 mt-3">
        <div v-for="item in filteredItems(values.search)" :key="item.id" class="card border-0 shadow-sm p-3">
          ...
          <div class="d-flex gap-2 mt-3">
            <Btn color="dark" icon="visibility" :to="{ name: 'TreatmentView', params: { id: item.id } }">Apri</Btn>
        ...
        <p v-if="!filteredItems(values.search).length" class="text-muted small mb-0">Nessun trattamento trovato.</p>
      </div>
    </Form>
  </div>
</template>
